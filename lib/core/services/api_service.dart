...
class ApiService {
  ...
  //
  // --- FUNGSI TOP SCORER (BARU) ---
  //
  Future<List<TopScorerModel>> getTopScorersByLeague(
    String leagueId,
    String season, // season sekarang adalah '2023', '2022', dll.
  ) async {
    final String leagueCode = _leagueCodes[leagueId] ?? 'PL';
    ...
    try {
      // Endpoint baru: /competitions/{leagueCode}/scorers?season={YYYY}
      final response = await _dio.get(
        '/competitions/$leagueCode/scorers',
        queryParameters: {'season': season}, // Kirim musim sebagai query
      );

      if (response.statusCode == 200 && response.data['scorers'] is List) {
        final List scorersList = response.data['scorers'];
        // Kita kirim data mentah ke TopScorerModel.fromJson
        return scorersList.map((e) => TopScorerModel.fromJson(e)).toList();
      } else {
        return []; // Kembalikan list kosong jika tidak ada data
      }
    } catch (e) {
